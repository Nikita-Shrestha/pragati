...
if(isset($_POST['submit']))
{
    ...
    $sql="UPDATE sregister SET name='$name',email='$email',contact='$contact',dob='$dob',gender='$gender',address='$address',batch='$batch',sem='$sem',status='$status' WHERE id='$id'";
...
<body>
    ...
    <form action="verify.php" method="POST"  style="border:1px solid #ccc" autocomplete="off">
    <?php
     $conn= new mysqli("localhost","root","","pragati");
    if($conn->connect_errno!=0)
    {
...
<div class="column">
    <div class="input-box">
        <input type="number" name="batch" id="batch" value="<?php echo $row['batch'] ?>" 
               placeholder="Enter the joined batch" min="2075" max="2080">
    </div>
    <div class="input-box">
        <input type="number" name="sem" id="sem" value="<?php echo $row['sem'] ?>" 
               placeholder="Enter the semester" min="1" max="7">
    </div>
</div>
